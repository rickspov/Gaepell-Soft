defmodule EvaaCrmGaepell.Document do
  ...
  def list_documents(business_id, opts \\ []) do
    ...
    |> filter_by_maintenance_ticket(opts[:maintenance_ticket_id])
  ...
  defp filter_by_maintenance_ticket(query, nil), do: query
  defp filter_by_maintenance_ticket(query, ticket_id) do
    where(query, [d], d.maintenance_ticket_id == ^ticket_id)
  end
  ...
  def get_document_stats(business_id) do
    ...
    basic_stats = from d in __MODULE__,
    ...
    # Obtener categorías
    categories_query = from d in __MODULE__,
      where: d.business_id == ^business_id,
      group_by: d.category,
      select: {d.category, count(d.id)}

    case EvaaCrmGaepell.Repo.one(basic_stats) do
      %{total_documents: total, total_files: files, total_size: size} ->
        categories = EvaaCrmGaepell.Repo.all(categories_query)
        |> Enum.into(%{})
